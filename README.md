...
**One-click official sign-in** — Open **Plugins → Installed → chatgpt-subscription**, then click **授权登录** → your browser opens OpenAI's official authorization page → approve → you're bound. No API keys, no command line, no config file editing.
...
- **Bind status page** — The plugin detail page shows bound/unbound status, token expiry and remaining time, with **授权登录 / 重新授权 / 解绑** actions.
...
The plugin auto-refreshes it before expiry. If refresh fails (e.g. revoked), the plugin detail page shows a clear hint to re-authorize.
