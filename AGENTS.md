...
`dsh-chatgpt-sub` 是 DeepSeek Harness (DSH) 的静态 bundle 插件：用官方 OAuth 绑定 ChatGPT 账号，在 DSH 内使用 ChatGPT 模型对话。

## 结构

- `src/host.js` — host 半：OAuth 绑定状态机（PKCE/state/回调/令牌交换/写回/绑定标记）、令牌看护（30min 周期 + JWT 过期判定 + 续期 + 凭据注入）、openai-codex 路由注册（displayName=ChatGPT, transport=sse）、RPC（`/_dsh/dsh-chatgpt-sub/{getCodexBridgeStatus,startCodexOAuth,unbindCodex}`，修改类同源防护）
- `src/client-bundle.js` — client 半：插件详情页「ChatGPT 订阅」配置（`plugins.bundle.config`，key=`dsh-chatgpt-sub`）
- `scripts/build.mjs` — 构建（src/ → lib/；client 包装为 `window.__ModuleLoader__.load({ id: "dsh-chatgpt-sub", ... })`）
...
- `cordis.patch.yml` — bundle 挂载行（`name` 必须是包名 `dsh-chatgpt-sub`；`id` 是用户覆盖用的行键，取短名 `chatgpt-subscription`，**不得与 name 相同**——相同的话插件卡片的「行 id / 模块名」两行会是同一串字）
...
1. **插件 id / client 模块 id 一律 `dsh-chatgpt-sub`**，禁止出现 `dsh-bottom-info-bar`（复制底稿残留会破坏挂载）
...
1. **插件页「添加插件」**（桌面端客户端唯一入口）——「包名或地址」框支持包名 / GitHub 仓库地址 / 本地目录绝对路径，三者本仓库**全部支持**（包名 = `dsh-chatgpt-sub`，npm 发布版本；仓库地址 = 仓库默认分支）。**仓库根必须就是插件包**（`package.json` + `cordis.patch.yml` 在根），且 `lib/` 已入库：这样填仓库地址零构建、零授权即可安装。
...
- **`package.json` 只允许 `prepublishOnly`**（它同时是 npm 发布时的重建钩子）。**禁止加 `prepare` / `prepack` / `postinstall`**：pnpm 对 git 依赖会执行 `prepare` 并在未放行时直接失败（`ERR_PNPM_GIT_DEP_PREPARE_NOT_ALLOWED`），会让「填 GitHub 地址」这条主流安装路径变成「待批准的构建脚本」卡点（已实测：带 `prepare` 的清单装 git 地址 exit=1，只留 `prepublishOnly` 则 exit=0 且零构建）。
- **包名已统一为 `dsh-chatgpt-sub`（2026-09-26 决定，用户拍板）**：GitHub 仓库、npm 包名、本地目录三处同名；npm 由 `publish-npm.yml` 随 tag 自动发布（secret `NPM_TOKEN`）。旧名 `dsh-chatgpt-subscription` 只允许出现在三处：绑定数据目录的一次性迁移逻辑（`src/host.js` 的 `LEGACY_DATA_DIR`）、`uninstall.sh` 的双名兼容、文档里的旧地址跳转说明——`test-install-docs.mjs` 钉住这三条边界。绑定标记目录随包名升级（`~/.dsh/dsh-chatgpt-sub`），旧目录在首次启动时自动搬移，用户不用重新绑定。
...
4. 发布四件套同步：semver → CHANGELOG → commit → tag → Release；tag 触发 `publish-npm.yml` 自动发布 npm，Release 同时是 npm 上架与安装地址（`…/dsh-chatgpt-sub`）的刷新节点
