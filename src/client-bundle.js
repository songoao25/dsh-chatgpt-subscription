...
module.exports = {
  ...
  async apply(ctx) {
    ...
    // 插件描述文案：DSH 内置语言只有 zh / en，取值交给 ctx.locale.resolveText（与官方包读
    // locale/*.json 的同一机制），取不到 locale 服务时退回浏览器语言，最后退回英文。
    // 这里的 description 与 locale/{en,zh}.json 的 meta.description 必须逐字一致：
    // 插件列表卡片读 locale 文件，详情页顶部读本文件的 summary 视图，两处都要对得上。
    var COPY = {
      title: { en: 'ChatGPT Subscription', zh: 'ChatGPT 订阅' },
      description: {
        en: "Bind your ChatGPT account through OpenAI's official OAuth flow, then use ChatGPT models in DSH with your subscription quota.",
        zh: '用 OpenAI 官方 OAuth 绑定 ChatGPT 账号，在 DSH 里使用 ChatGPT 模型对话，消耗你的订阅额度。',
      },
    };
    function pickText(text) {
      var locale = ctx.locale || (ctx.get && ctx.get('locale'));
      if (locale && typeof locale.resolveText === 'function') {
        try {
          var resolved = locale.resolveText(text);
    ...
    function SubscriptionPage(props) {
      ...
      // 只放标题——「这是什么插件」由详情页顶部的描述负责，这里不再复述。
      var head = h('div', { className: 'cgpt-sectionHead' },
        h('h4', { className: 'cgpt-title' }, pickText(COPY.title)));
    ...
    function SubscriptionBundleConfig(props) {
      if (props && props.view === 'summary') {
        return h('span', { className: 'dshChatGPTBundleSummary' }, pickText(COPY.description));
      }
      ...
      return slots.register(
        {
          ...
          label: function () { return pickText(COPY.title); },
        },
