...
import { createHostTranslator } from './host-locale.js'
...
function startOAuthCallbackServer(expectedState, onCode, port, t = createHostTranslator()) {
  return new Promise(function (resolve) {
    ...
    const server = createServer(function (req, res) {
      ...
      try {
        ...
        respondOAuthPage(res, 400, t('host.invalidOAuthCallbackURL'), t)
        return
      }
      if (pathname !== OAUTH_CALLBACK_PATH) {
        respondOAuthPage(res, 404, t('host.callbackPathNotFound'), t)
        return
      }
      if (params.get('state') !== expectedState) {
        respondOAuthPage(res, 400, t('host.oauthStateVerificationFailedPlease'), t)
        return
      }
      const code = params.get('code')
      if (!code) {
        respondOAuthPage(res, 400, t('host.authorizationCodeMissing'), t)
        return
      }
      respondOAuthPage(res, 200, t('host.authorizationReceivedYouCanClose'), t)
      onCode(code)
    })
...
function respondOAuthPage(res, status, message, t = createHostTranslator()) {
  const safe = String(message).replace(/[&<>"']/g, function (c) {
    return { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]
  })
  const html = '<!DOCTYPE html><html lang="' + t('host.pageLanguage') + '"><head><meta charset="utf-8"><title>' + t('host.pageTitle') + '</title></head>' +
    ...
    '<p style="color:#555">' + t('host.pageCloseHint') + '</p>' +
    '</body></html>'
...
export default {
  ...
  apply(ctx) {
    const t = createHostTranslator(ctx);
    ...
    async function ensureCodexRoute(flag) {
      ...
      if (!settings || typeof settings.get !== 'function' || typeof settings.mutate !== 'function') {
        return { ok: false, owned: false, message: t('host.settingsServiceIsNotReady') };
      }
      try {
        ...
        if (existing && existing.apiKeyEnv !== 'OPENAI_CODEX_API_KEY') {
          return { ok: false, owned: false, message: t('host.aCustomOpenaiCodexRoute') };
        }
        if (existing) {
        ...
        return { ok: false, owned: false, message: t('host.couldNotRegisterTheOpenai') };
      }
    }
    ...
    async function configureDefaultModel() {
      ...
      if (!service || typeof service.currentSelection !== 'function' || typeof service.saveSelection !== 'function') return { ok: false, previous: null, message: t('host.theDefaultModelServiceIs') };
    ...
    async function syncCodexToken() {
      ...
      syncInFlight = syncCodexTokenOnce().catch(function () {
        codexBridgeState = { ok: false, lastSyncAt: Date.now(), expiresAt: null, error: { kind: 'exception', message: t('host.tokenSyncFailed') }, routeConfigured: codexBridgeState.routeConfigured };
    ...
    async function syncCodexTokenOnce() {
      ...
      if (!flag.bound) {
        ...
        try { await removeOwnedCodexRoute(flag); codexRouteOwned = false; } catch (err) {
          codexBridgeState = { ok: false, lastSyncAt: nowMs, expiresAt: null, error: { kind: 'route-cleanup', message: t('host.couldNotRemoveTheRoute') }, routeConfigured: true };
          return;
        }
        codexBridgeState = { ok: false, lastSyncAt: nowMs, expiresAt: null, error: cleared ? { kind: 'unbound', message: t('host.notConnectedSignInOn') } : { kind: 'credentials-cleanup', message: t('host.notConnectedButOldCredentials') }, routeConfigured: false };
        return;
      }
      ...
      if (!read.ok) {
        ...
        const error = !routeRemoved ? { kind: 'route-cleanup', message: t('host.connectionIsNoLongerValid') } : cleared ? { kind: 'no-login', message: t('host.connectionIsNoLongerValid.error') } : { kind: 'credentials-cleanup', message: t('host.signInCredentialsAreMissing') };
      ...
      if (!access) {
        ...
        let routeRemoved = true;
        try { await removeOwnedCodexRoute(flag); codexRouteOwned = false; } catch (err) { routeRemoved = false; }
        const error = !routeRemoved ? { kind: 'route-cleanup', message: t('host.connectionIsNoLongerValid') } : cleared ? { kind: 'no-key', message: t('host.connectionIsNoLongerValid.error2') } : { kind: 'credentials-cleanup', message: t('host.accessTokenIsMissingAnd') };
        codexBridgeState = { ok: false, lastSyncAt: nowMs, expiresAt: null, error: error, routeConfigured: !routeRemoved };
        return;
      }
      ...
      if (codexNeedsRefresh(expiresAtSec, nowSec)) {
        if (!refresh) {
          error = { kind: 'auth', message: t('host.tokenExpiresSoonButRefresh') };
        } else {
          ...
          if (pair) {
            try {
              ...
              error = { kind: 'write', message: t('host.tokenRefreshedButCouldNot') };
            }
          } else {
            ...
            if (reAuth && reAccess && !isNaN(reRefreshMs) && (isNaN(lastRefreshMs) || reRefreshMs > lastRefreshMs)) {
              ...
              error = { kind: 'auth', message: t('host.tokenRefreshFailedRefreshToken') };
            }
          }
        }
      } else {
      ...
      if (!token && error) {
        const cleared = await clearInjectedCodexCredential(flag);
        if (!cleared) error = { kind: 'credentials-cleanup', message: t('host.tokenIsUnusableAndOld') };
      }
      if (token && token !== codexInjectedToken) {
        const canClaim = await canClaimCodexCredential(flag);
        if (!canClaim) {
          error = { kind: 'credentials-conflict', message: t('host.openaiCODEXAPIKEYIs') };
        } else try {
          ...
          const cleared = await clearInjectedCodexCredential(flag);
          error = cleared ? { kind: 'credentials', message: t('host.couldNotSetDSHCredentials') } : { kind: 'credentials-cleanup', message: t('host.couldNotSetDSHCredentials.syncCodexTokenOnce') };
        }
      }
    ...
    async function runCodexOAuthFlow(pkce, state, serverHandle, authorizeUrl, waitCode) {
      ...
      try {
        ...
        if (!code) { oauthLastError = { kind: 'timeout', message: t('host.authorizationTimedOutAfterMinutes') }; return; }
        const exchanged = await exchangeAuthorizationCode(code, pkce.verifier);
        if (!exchanged.ok) {
          oauthLastError = { kind: 'exchange', message: t('host.tokenExchangeFailed') + (exchanged.status != null ? 'HTTP ' + exchanged.status : t('host.networkError')) + t('host.pleaseTryAgain') };
          return;
        }
        ...
        try {
          ...
          oauthLastError = { kind: 'write', message: t('host.authorizedButCouldNotSave') };
          return;
        }
        ...
        try {
          ...
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
          oauthLastError = { kind: 'default-model', message: t('host.authorizedButCouldNotSwitch') };
          return;
        }
        if (!defaultModel.ok) {
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
        ...
        if (!canClaim) {
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
          oauthLastError = { kind: 'credentials-conflict', message: t('host.openaiCODEXAPIKEYIs') };
          return;
        }
        try {
          ...
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
          oauthLastError = { kind: 'write', message: t('host.authorizedButCouldNotSave.runCodexOAuthFlow') };
          return;
        }
        try {
          ...
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
          oauthLastError = { kind: 'credentials', message: t('host.connectionFailedCouldNotSet') };
        ...
        await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
        oauthLastError = { kind: 'exception', message: t('host.connectionFailedPleaseTryAgain') };
      } finally {
    ...
    async function startCodexOAuthRpc() {
      if (oauthInFlight) return { ok: false, oauthInFlight: true, error: { kind: 'in-flight', message: t('host.authorizationInProgressPleaseWait') } };
      oauthInFlight = true;
      oauthLastError = null;
      ...
      const serverHandle = await startOAuthCallbackServer(state, function (code) { waitCode.resolve(code); }, port, t);
      if (!serverHandle) {
        oauthInFlight = false;
        return { ok: false, oauthInFlight: false, error: { kind: 'port-busy', message: t('host.callbackPortIsInUse', { port: port }) } };
      }
    ...
    async function unbindCodexRpc() {
      if (oauthInFlight) return { ok: false, error: { kind: 'in-flight', message: t('host.authorizationInProgressCompleteIt') } };
      try {
        const flag = readBindFlag(CODEX_BIND_FILE);
        ...
        codexBridgeState = { ok: false, lastSyncAt: Date.now(), expiresAt: null, error: { kind: 'unbound', message: t('host.chatgptSubscriptionDisconnectedSignIn') }, routeConfigured: false };
        return { ok: true, bound: false };
      } catch (err) {
        return { ok: false, error: { kind: 'exception', message: t('host.couldNotDisconnectPleaseTry') } };
      }
    }
    ...
    function respond(res, status, payload) {
      ...
      res.end(JSON.stringify(payload, t.json));
    }
